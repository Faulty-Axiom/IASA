...
class RespostaEvitar(Resposta):
    ...
    '''
    Método que determina a ação de evasão, retornando uma ação de rodar para uma direção livre
    '''
    def _obter_accao(self, percepcao: Percepcao) -> Accao:
        # Procurar a primeira direção que não tenha um obstáculo imediatamente à frente
        for direccao in Direccao:
            elemento, distancia, posicao = percepcao[direccao]
            if elemento != Elemento.OBSTACULO:
